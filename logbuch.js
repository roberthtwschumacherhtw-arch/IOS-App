...
.traintimer{position:fixed;right:12px;bottom:calc(80px + env(safe-area-inset-bottom));z-index:40;display:flex;align-items:center;gap:14px;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:8px 13px;box-shadow:0 8px 24px rgba(15,23,32,.22)}
...
.tt-val{font-family:var(--mono);font-size:18px;font-weight:500;font-variant-numeric:tabular-nums;color:var(--ink)}
...
.seclab{font:600 10px/1 var(--sans);letter-spacing:.12em;text-transform:uppercase;color:var(--ink-30);margin:26px 0 -2px;padding:0 2px}
...
let db = {exercises:[...DEFAULT_EX], workouts:[], body:[], splits:[], exGroups:{}, exNotes:{}, customBarcodes:{}, sex:'m', age:null, height:null, n8nUrl:'', mealTypes:[...DEFAULT_MEALS], goals:{...DEFAULT_GOALS}, foodFav:[], nutrition:{...DEFAULT_NUTRITION}, ui:{theme:'auto'}};
...
function empiricalCoverage(){
  ...
  const kd = b.filter(x=>x.kcal!=null);
  const T = s => new Date(s+'T12:00:00').getTime();
  const span = ws.length>1 ? (T(ws[ws.length-1].date)-T(ws[0].date))/864e5 : 0;
  return { weights:ws.length, spanDays:Math.round(span), kcalDays:kd.length,
           enough: ws.length>=EMP_MIN_WEIGHTS && span>=EMP_MIN_SPAN_D && kd.length>=EMP_MIN_KCAL_DAYS };
}
...
function calorieTarget(){
  ...
  if(maint!=null) return { kcal: Math.round(maint + phaseDelta()), source:'auto', phase:n.phase, maint:Math.round(maint) };
...
function renderGoals(){
  ...
  if(shown.length){
    ...
    html += `<div class="empty">Noch keine Sätze diese Woche</div>`;
  }
  host.innerHTML = html;
}
...
function weightChart(pts, sel){
  ...
  if(!pts.length) return {svg:'<div class="empty">Noch kein Gewicht im Zeitraum</div>', pts:[], px:[], py:[]};
...
function showView(v){
  ...
  if(v==='food') renderMeals();
}
{ var _hb=$('#homeBtn'); if(_hb) _hb.onclick=function(){ showView('home'); }; }
{ var _eb=$('#essenBtn'); if(_eb) _eb.onclick=function(){ showView('food'); }; }
function renderHome(){
  var host=$('#homeBody'); if(!host) return;
  var b=bodySorted(), ws=b.filter(function(x){return x.weight!=null;});
  var m=function(a){return a.length?a.reduce(function(x,y){return x+y;},0)/a.length:null;};
  var cur=m(ws.slice(-7).map(function(x){return x.weight;})), prev=m(ws.slice(-14,-7).map(function(x){return x.weight;}));
  var e=db.body.find(function(x){return x.date===TODAY;})||{};
  var tg=macroTargets(), kt=tg.kcal, kc=Math.round(e.kcal||0);
  var R=54,C=Math.round(2*Math.PI*R),frac=kt?Math.min(1,kc/kt):0,over=kt&&kc>kt,dash=Math.round(C*frac);
  var left=kt!=null?(over?'+'+(kc-kt):Math.max(0,kt-kc)):'—';
  var lab=kt==null?'KEIN ZIEL':(over?'KCAL ÜBER':'KCAL ÜBRIG');
  var ring='<svg width="112" height="112" viewBox="0 0 128 128"><circle cx="64" cy="64" r="'+R+'" fill="none" stroke="var(--grid)" stroke-width="13"></circle>'+(dash>0?'<circle cx="64" cy="64" r="'+R+'" fill="none" stroke="'+(over?'var(--signal)':'var(--accent)')+'" stroke-width="13" stroke-linecap="round" stroke-dasharray="'+dash+' '+C+'" transform="rotate(-90 64 64)"></circle>':'')+'<text x="64" y="59" text-anchor="middle" class="ring-big">'+left+'</text><text x="64" y="79" text-anchor="middle" class="ring-lab">'+lab+'</text></svg>';
  var wdelta=(prev!=null&&cur!=null)?((cur>=prev?'+':'')+round(cur-prev,1)+' kg'):'—';
  var sv=ws.slice(-10).map(function(x){return x.weight;}), spark='';
  if(sv.length>1){ var mn=Math.min.apply(null,sv),mx=Math.max.apply(null,sv),sp=(mx-mn)||1; var pts=sv.map(function(v,i){var x=2+i*116/(sv.length-1);var y=4+(1-(v-mn)/sp)*22;return x.toFixed(1)+','+y.toFixed(1);}).join(' '); spark='<svg width="100%" height="30" viewBox="0 0 120 30" preserveAspectRatio="none" style="margin-top:8px;display:block"><polyline points="'+pts+'" fill="none" stroke="var(--teal)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></polyline></svg>'; }
  var nd=new Date(),dow=(nd.getDay()+6)%7,mon=new Date(nd.getFullYear(),nd.getMonth(),nd.getDate()-dow).getTime(),sun=mon+7*864e5;
  var inW=function(d){var t=new Date(d+'T12:00:00').getTime();return t>=mon&&t<sun;};
  var ssn={}; for(var i=0;i<db.workouts.length;i++){ var w=db.workouts[i]; if(inW(w.date)) ssn[w.sessionId||w.id]=1; }
  var sessions=Object.keys(ssn).length, gs=(db.goals&&db.goals.sessions)||3;
  var gp=Math.min(100,Math.round(sessions/gs*100)), gdone=sessions>=gs;
  var goalHtml='<div class="goal"><div class="goal-top"><span class="goal-t">Gym-Einheiten</span><span class="goal-v"><b>'+sessions+'</b> / '+gs+'</span></div><div class="goal-bar"><i class="'+(gdone?'done':'')+'" style="width:'+gp+'%"></i></div></div>';
  var tw=db.workouts.filter(function(w){return w.date===TODAY;}), trainHtml;
  if(tw.length){ var sets=tw.reduce(function(a,w){return a+(w.sets?w.sets.length:0);},0); var exs=tw.map(function(w){return esc(w.exercise);}).join(' · '); trainHtml='<div style="font-family:var(--mono);font-size:13px;color:var(--ink-60);margin-bottom:6px"><b style="color:var(--ink)">'+tw.length+'</b> Übungen · <b style="color:var(--ink)">'+sets+'</b> Sätze</div><div style="font-size:12.5px;color:var(--ink-30)">'+exs+'</div>'; }
  else { trainHtml='<div style="font-size:13px;color:var(--ink-30)">Noch kein Training heute — starte im Tab „Training".</div>'; }
  host.innerHTML =
    '<div class="grid2">'
    +'<div class="card ringcard" style="display:flex;flex-direction:column;align-items:center;justify-content:center">'+ring+'<div style="font-family:var(--mono);font-size:12px;color:var(--ink-60);margin-top:6px"><b style="color:var(--ink)">'+kc+'</b> / '+(kt||'—')+' kcal</div></div>'
    +'<div class="card wcard"><div style="font:600 10px/1 var(--sans);letter-spacing:.12em;text-transform:uppercase;color:var(--ink-30);margin-bottom:10px">Gewicht</div><div style="font-family:var(--mono);font-size:24px;font-weight:600">'+(cur!=null?round(cur,1):'—')+'<span style="font-size:12px;color:var(--ink-30)"> kg</span></div><div style="font-family:var(--mono);font-size:12px;font-weight:600;color:var(--teal);margin-top:5px">'+wdelta+'</div>'+spark+'</div>'
    +'</div>'
    +'<div class="card compact" style="margin-top:12px"><h2>Wochenziele</h2>'+goalHtml+'</div>'
    +'<div class="card" style="margin-top:12px"><div class="meals-head" style="margin-bottom:10px"><span class="t">Heute trainiert</span></div>'+trainHtml+'</div>';
}
...
function renderAll(){
  ...
  renderMeals(); renderNutri(); renderGoals(); renderHome();
}
...
function renderNutri(){
  ...
  else { const dl=phaseDelta(); kt=tg.kcal; const est=tg.info.source==='formel'; metaStr=phaseLabel+(tg.info.maint?' · '+(est?'geschätzt':'Erhalt')+' ~'+tg.info.maint:'')+(dl?' · '+(dl>0?'+':'')+dl+' kcal':'')+(est?' · Schätzung':''); }
  ...
  if(ringHost){
    ...
    if(pill){
      ...
      pill.title='Zu den Phasen-Einstellungen';
      pill.onclick=()=>{
        const row=$('.phaserow'); if(!row) return;
