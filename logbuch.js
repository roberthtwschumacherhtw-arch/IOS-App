...
.traintimer{bottom:calc(74px + env(safe-area-inset-bottom))}
...
/* Ziel-Sheet */
#goalOv .picksheet{max-height:86vh;overflow-y:auto}
...
$('#daySel').addEventListener('change', ()=>{ loadDay(); updateUnitPill(); });
...
function renderDataStats(){
  ...
  host.innerHTML=tile(sessions,'Einheiten')+tile(measures,'Messungen')+tile(db.exercises.length,'Übungen')+tile(entries,'Einträge');
...
function renderAll(){
  ...
  renderMeals(); renderNutri(); renderGoals(); renderHome(); renderDataStats(); updateUnitPill();
}
