...
/* --- Tagebuch: Wasser, Umfaenge und Notiz kompakt in einer Karte --- */
.diary .quickin{margin:0 0 12px}
.drow{border-top:1px solid var(--grid);padding:11px 0 2px}
.drow:last-child{padding-bottom:0}
.drow-h{display:flex;align-items:baseline;justify-content:space-between;gap:10px;margin-bottom:7px}
.dh-t{font:650 13.5px var(--sans)}
.dh-v{font-family:var(--mono);font-size:12.5px;color:var(--ink-60)}
.dh-s{font-family:var(--mono);font-size:11.5px;color:var(--ink-30);line-height:1.45}
.drow .goal-bar{margin-bottom:9px}
.dchips{display:flex;gap:6px;flex-wrap:wrap}
/* Timerzeile im klassischen Loggen */
#unitTimer{display:flex;gap:10px;margin:0 0 12px}
#unitTimer .ut-i{flex:1;background:var(--field);border:1px solid var(--line);border-radius:11px;padding:8px 11px}
#unitTimer .ut-l{display:block;font:600 9.5px/1 var(--sans);letter-spacing:.12em;text-transform:uppercase;color:var(--ink-30)}
#unitTimer .ut-v{display:block;font-family:var(--mono);font-size:16px;font-variant-numeric:tabular-nums;margin-top:4px}
#unitTimer.over .ut-i:last-child{border-color:var(--signal);background:color-mix(in srgb,var(--signal) 12%,var(--card))}
#unitTimer.over .ut-i:last-child .ut-v{color:var(--signal)}
/* Historie zugeklappt */
.histtog{width:100%;background:none;border:0;padding:0;display:flex;align-items:center;justify-content:space-between;gap:10px;min-height:var(--tap);color:var(--ink)}
.histtog h2{margin:0}
.histchev{color:var(--ink-30);font-size:13px}
.atcancel{color:var(--signal)!important;margin-left:12px}
.dchip{
  flex:1 1 auto;min-width:52px;padding:9px 6px;font:600 12.5px var(--sans);
  background:var(--field);color:var(--ink);border:1px solid var(--line);border-radius:9px;min-height:40px
}
#noteCard input{font-size:14px;padding:10px 12px;min-height:42px}
...
function freshDb(){
  return {exercises:[...DEFAULT_EX], workouts:[], body:[], splits:[], exGroups:{}, exNotes:{},
          ...
          nutrition:{...DEFAULT_NUTRITION}, ui:{theme:'auto', activeMode:true}, lastExport:null};
}
let db = freshDb();
...
function addBlock(exName, prefill=true, data=null){
  ...
  if(typeof activeMode==='function' && !activeMode()){
    b.dataset.open='1';
    const t=b.querySelector('.blocktog');
...
function markSet(){
  const now=Date.now();
  if(!trainState.startAt) trainState.startAt=now;
  trainState.lastSetAt=now;
  saveTrainState(); updateTimer();
  if(typeof beepArm==='function' && typeof activeMode==='function' && !activeMode()) beepArm();
  if(typeof renderUnitTimer==='function') renderUnitTimer();
}
// Beim klassischen Loggen gibt es kein Vollbild — Dauer und Pause stehen
// deshalb als schmale Zeile in der Einheit-Karte. Im aktiven Modus zeigt sie
// nichts, dort uebernimmt das Overlay.
function renderUnitTimer(){
  const el=$('#unitTimer'); if(!el) return;
  const zeigen = typeof activeMode==='function' && !activeMode() && !!trainState.startAt;
  el.style.display = zeigen ? 'flex' : 'none';
  if(!zeigen) return;
  const now=Date.now();
  const dauer=fmtDur(now-trainState.startAt);
  const ziel=(db.goals && db.goals.restSec) || 120;
  let pauseTxt='—', ueber=false;
  if(trainState.lastSetAt){
    const sek=Math.floor((now-trainState.lastSetAt)/1000);
    ueber = sek>=ziel;
    pauseTxt=fmtDur(now-trainState.lastSetAt)+' / '+Math.floor(ziel/60)+':'+String(ziel%60).padStart(2,'0');
    if(ueber && typeof beep==='function') beep();
  }
  el.classList.toggle('over', ueber);
  el.innerHTML='<span class="ut-i"><span class="ut-l">Einheit</span><span class="ut-v">'+dauer+'</span></span>'
             + '<span class="ut-i"><span class="ut-l">Pause</span><span class="ut-v">'+pauseTxt+'</span></span>';
}
...
{ $$('.js-start').forEach(b=>b.onclick=startTraining);
  const cw=$('#cancelW'); if(cw) cw.onclick=cancelTraining;
  const so=$('#startOv');
  if(so){
  ...
  const rb=$('#resumeBar');
  if(rb) rb.querySelector('.rb-go').onclick=()=>{
    const nb=$$('nav button').find(x=>x.dataset.v==='log'); if(nb) nb.click();
    openLogging(); renderResumeBar();
  };
  setInterval(renderResumeBar, 1000);
  const seg=$('#modeSeg');
  if(seg) seg.querySelectorAll('button').forEach(b2=>b2.onclick=async()=>{
    db.ui={...(db.ui||{}), activeMode: b2.dataset.m==='active'};
    await Store.save(db);
    renderModeSeg();
    if(!activeMode()) oeffneBloecke();
    toast(activeMode()?'Aktives Training':'Klassisches Loggen');
  });
  renderModeSeg();
  const rs=$('#restSec');
  if(rs){
...
trainOv.innerHTML = `
  <div class="atwrap">
    <div class="athead">
      ...
      <button class="link warn atcancel">Abbrechen</button>
...
trainOv.querySelector('.atcancel').onclick = ()=>{ cancelTraining(); };
...
// Eine Einheit verwerfen, ohne sie zu speichern. Bewusst mit Rueckfrage und
// bewusst nicht im Speichern-Knopf versteckt — das sind zwei gegensaetzliche
// Absichten, die nicht am selben Ort stehen sollten.
function cancelTraining(){
  const n = unsavedSets();
  if(n && !confirm('Training abbrechen?\n\n'+n+(n===1?' Satz geht':' Sätze gehen')+' verloren. Nichts wird gespeichert.')) return false;
  closeAT();
  $('#blocks').innerHTML='';
  editingSession = null; editingDayName = null;
  $('#saveW').textContent = 'Einheit speichern';
  ...
  $('#dayHint').textContent = '';
  endSession();
  markUnsaved(); renderTrainProgress(); renderResumeBar();
...
// Zwei Wege zum Loggen: der aktive Vollbildmodus oder die klassische Liste im
// Training-Tab. Umschaltbar in den Einstellungen — die Daten sind identisch,
// nur die Oberflaeche unterscheidet sich.
function activeMode(){ return (db.ui||{}).activeMode !== false; }
// Muss nach dem Laden der Daten erneut laufen: beim Skriptstart steht db noch
// auf den Vorgabewerten, der gespeicherte Modus kommt erst mit Store.load().
function renderModeSeg(){
  const seg=$('#modeSeg');
  const a=activeMode();
  if(seg) seg.querySelectorAll('button').forEach(x=>x.setAttribute('aria-pressed', String((x.dataset.m==='active')===a)));
  const h=$('#modeHint');
  if(h) h.textContent = a
    ? 'Vollbild mit einer Übung im Fokus, Satz abhaken und Pausentimer.'
    : 'Alle Übungen als Liste im Training-Tab, wie vor dem Umbau.';
}
function oeffneBloecke(){
...
function openLogging(){
  if(activeMode()){ openAT(); return; }
  oeffneBloecke();
  const uc=$('#unitCard');
  if(uc && uc.scrollIntoView) uc.scrollIntoView({behavior:'smooth', block:'start'});
}
async function startTraining(){
  const nb = $$('nav button').find(b=>b.dataset.v==='log');
  if(nb) nb.click();
  // Laeuft schon etwas, wird nicht gefragt, sondern fortgesetzt.
  if(atBlocks().length){ openLogging(); return; }
...
function renderTrainProgress(){
  ...
  // Die Einheit-Karte erscheint erst, wenn wirklich eine Einheit laeuft.
  // Vorher stand dort ein leeres Formular, das nur Platz gekostet hat.
  const uc = $('#unitCard');
  if(uc) uc.style.display = gesamt ? 'block' : 'none';
  renderUnitTimer();
...
function renderAnalysis(){
  ...
  ].concat(kt?[{pts:daysK.map(x=>({x:T(x.date),y:kt})), color:'var(--ink-30)', w:1.4, dash:true}]:[]), {zero:true}) : '<div class="emptybox"><div class="eb-t">Keine Kalorien erfasst</div><div class="eb-s">Trage Mahlzeiten ueber die Produktsuche ein.</div></div>';
  ...
  $('#kcalStats').innerHTML = daysK.length ? `
    ...
    <div class="stat"><div class="k">Über / Unter</div><div class="v" style="font-size:17px">${over} / ${under}</div><div class="s">Tage</div></div>` : '<div class="emptybox"><div class="eb-t">Keine Kalorien erfasst</div><div class="eb-s">Trage Mahlzeiten ueber die Produktsuche ein.</div></div>';
...
// Die Historie startet zugeklappt — sie ist Nachschlagewerk, nicht Alltag.
{ const t=$('#histTog'), b=$('#histBody');
  if(t && b) t.onclick=()=>{
    const auf = b.style.display !== 'none';
    b.style.display = auf ? 'none' : 'block';
    t.setAttribute('aria-expanded', String(!auf));
    const ch=t.querySelector('.histchev'); if(ch) ch.textContent = auf ? '▾' : '▴';
  };
}
...
function renderAll(){
  ...
  if(typeof renderModeSeg==='function') renderModeSeg();
  if(typeof renderTrainProgress==='function') renderTrainProgress();
}
...
function renderWater(){
  ...
  host.innerHTML =
      '<div class="drow-h"><span class="dh-t">Wasser</span>'
    +   '<span class="dh-v">'+dec1(ml/1000)+' / '+dec1(t/1000)+' l</span></div>'
    + '<div class="goal-bar"><i class="'+(ml>=t?'done':'')+'" style="width:'+pct+'%;background:var(--blue)"></i></div>'
    + '<div class="dchips">'
    +   [250,500,750].map(v=>'<button class="dchip" data-w="'+v+'">+'+v+'</button>').join('')
    +   '<button class="dchip" data-w="-250">−250</button>'
    +   '<button class="dchip" data-w="0" aria-label="Wasser zurücksetzen">↺</button>'
    + '</div>';
...
function renderMeas(){
  ...
  // Kompakt: eine Zeile mit den zuletzt gemessenen Werten, Details im Sheet.
  const werte = last
    ? MEAS_SITES.filter(m=>last.meas[m.k]!=null).map(m=>{
        const v=last.meas[m.k];
        const p=prev && prev.meas && prev.meas[m.k]!=null ? prev.meas[m.k] : null;
        const d=p!=null ? (v-p) : null;
        return m.label+' '+dec1(v)+(d!=null&&Math.abs(d)>=0.05 ? ' ('+(d>0?'+':'')+round(d,1)+')' : '');
      }).join(' · ')
    : '';
  host.innerHTML='<div class="drow-h"><span class="dh-t">Umfänge</span>'
    + '<button class="link" id="measEdit">'+(last?'eintragen':'jetzt messen')+'</button></div>'
    + '<div class="dh-s">'+(werte ? esc(werte)+' · '+fmtDate(last.date)
        : 'Noch nichts gemessen — Bauchumfang allein reicht schon.')+'</div>';
...
function renderDayNote(){
  ...
  ta.value=e.note||'';
  const meta=$('#noteMeta');
  if(meta) meta.textContent = fmtDate(d);
}
...
(async function init(){
  ...
  if(saved) db = {
    ...
    ui: {theme:'auto', activeMode:true, ...(saved.ui||{})}
  };
