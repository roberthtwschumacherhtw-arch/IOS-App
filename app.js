...
let db = {exercises:[...DEFAULT_EX], workouts:[], body:[], splits:[]};
...
const MUSCLE_RULES = [
  ...
  ['Bauch', ['bauch','crunch','plank','situp','sit-up','abs','core','rumpf','beinheben']],
...
function renderExList(){
  $('#exlist').innerHTML = db.exercises.map(e=>{
    ...
    return `<li><div class="li-main"><div class="li-t">${esc(e)}</div><div class="li-s">${n} Einheiten</div></div>
      <div class="li-d">${n?'':`<button class="link warn" data-exdel="${esc(e)}">entfernen</button>`}</div></li>`;
  }).join('');
...
$('#impJson').onchange = async e=>{
  ...
  try{
    ...
    db = {exercises:d.exercises?.length?d.exercises:[...DEFAULT_EX], workouts:d.workouts, body:d.body, splits:d.splits||[]};
