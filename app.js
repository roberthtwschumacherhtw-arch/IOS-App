...
let db = {exercises:[...DEFAULT_EX], workouts:[], body:[], splits:[], exGroups:{}, exNotes:{}, customBarcodes:{}, sex:'m', n8nUrl:'', mealTypes:[...DEFAULT_MEALS], goals:{...DEFAULT_GOALS}, foodFav:[], nutrition:{...DEFAULT_NUTRITION}};
...
function renderBody(){
  { const ss=$('#sexSeg'); if(ss) ss.querySelectorAll('button').forEach(b=>b.setAttribute('aria-pressed', String(b.dataset.sex===(db.sex||'m')))); }
  const b = bodySorted();
...
function weightChart(pts, sel){
  ...
  if(!pts.length) return {svg:'<div class="empty">Noch kein Gewicht im Zeitraum</div>', pts:[]};
  ...
  pts.forEach((p,i)=>{
    ...
    g+='<circle class="wpt" data-i="'+i+'" cx="'+cx+'" cy="'+cy+'" r="3.6" fill="var(--card)" stroke="var(--blue)" stroke-width="2"/>';
    if(labelSet.has(i) && i!==sel) g+='<text x="'+cx+'" y="'+(Y(p.y)-9).toFixed(1)+'" text-anchor="middle" font-family="var(--mono)" font-size="10.5" font-weight="600" fill="var(--blue)">'+p.y.toFixed(1)+'</text>';
  });
  if(sel!=null && pts[sel]){
    const p=pts[sel], cx=X(p.x);
    g+='<line x1="'+cx.toFixed(1)+'" y1="'+PT+'" x2="'+cx.toFixed(1)+'" y2="'+(H-PB).toFixed(1)+'" stroke="var(--ink-30)" stroke-width="1.5"/>';
    g+='<circle cx="'+cx.toFixed(1)+'" cy="'+Y(p.y).toFixed(1)+'" r="5.5" fill="var(--blue)" stroke="var(--card)" stroke-width="2.5"/>';
    const bw=124,bh=38; let bx=cx-bw/2; bx=Math.max(PL,Math.min(W-PR-bw,bx)); const by=4;
    g+='<rect x="'+bx.toFixed(1)+'" y="'+by+'" width="'+bw+'" height="'+bh+'" rx="9" fill="var(--ink)"/>';
    g+='<text x="'+(bx+11)+'" y="'+(by+17)+'" font-family="var(--mono)" font-size="14" font-weight="600" fill="var(--bg)">'+p.y.toFixed(1)+' kg</text>';
    g+='<text x="'+(bx+11)+'" y="'+(by+31)+'" font-family="var(--mono)" font-size="10" fill="var(--ink-30)">'+fmtDate(p.date)+'</text>';
  }
  pts.forEach((p,i)=>{ g+='<circle class="whit" data-i="'+i+'" cx="'+X(p.x).toFixed(1)+'" cy="'+Y(p.y).toFixed(1)+'" r="14" fill="transparent"/>'; });
  ...
  return {svg:'<svg class="chart wchart" viewBox="0 0 '+W+' '+H+'" role="img">'+g+'</svg>', pts};
}
function renderAnalysis(){
  ...
  const _drawW=()=>{
    $('#c-weight').innerHTML = weightChart(wpts, _wsel).svg;
    if(_wselEl){ const p=wpts[_wsel]; _wselEl.innerHTML = p ? '<span class="wsel-hint">Tippe einen Punkt für den Tageswert</span>' : '<span class="wsel-hint">Noch kein Gewicht im Zeitraum</span>'; }
    $$('#c-weight .whit').forEach(c=>c.onclick=()=>{ _wsel=+c.dataset.i; _drawW(); });
  };
  _drawW();
...
{ const ss=$('#sexSeg'); if(ss) ss.querySelectorAll('button').forEach(b=>b.onclick=async()=>{ db.sex=b.dataset.sex; ss.querySelectorAll('button').forEach(x=>x.setAttribute('aria-pressed', String(x.dataset.sex===db.sex))); await Store.save(db); renderNutri(); }); }
function renderAll(){
  fillDaySel();
  fillAnEx();
