...
function renderAnalysis(){
  ...
  $('#c-kcaltarget').innerHTML = daysK.length ? chart([
    {pts:daysK.map(x=>({x:T(x.date),y:x.kcal})), color:'var(--teal)', type:'bar', op:.5},
  ...
  if(ap||af||ac){
    $('#macroSplit').innerHTML='<div style="display:flex;align-items:center;gap:18px">'+donut([{v:kp/ktot,color:'var(--teal)'},{v:kf/ktot,color:'var(--ochre)'},{v:kc2/ktot,color:'var(--accent)'}])
      +'<div class="macrolegend">'
      +'<div class="ml"><i style="background:var(--teal)"></i><b>'+round(ap,0)+' g</b> Protein<span>'+Math.round(kp/ktot*100)+'%</span></div>'
      +'<div class="ml"><i style="background:var(--ochre)"></i><b>'+round(af,0)+' g</b> Fett<span>'+Math.round(kf/ktot*100)+'%</span></div>'
      +'<div class="ml"><i style="background:var(--accent)"></i><b>'+round(ac,0)+' g</b> KH<span>'+Math.round(kc2/ktot*100)+'%</span></div>'
  ...
  $('#c-steps').innerHTML = stepsArr.length ? chart([
    {pts:b.map(x=>({x:T(x.date),y:x.steps!=null?x.steps:null})).filter(p=>p.y!=null), color:'var(--blue)', type:'bar', op:.4},
    {pts:b.map((x,i)=>({x:T(x.date),y:st7[i]})).filter(p=>p.y!=null), color:'var(--blue)', w:2},
