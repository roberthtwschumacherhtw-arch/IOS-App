...
let db = {exercises:[...DEFAULT_EX], workouts:[], body:[], splits:[], exGroups:{}, n8nUrl:''};
...
function chart(series, opts={}){
  ...
  const A=axes.l;
  for(let i=0;i<=3;i++){
    const val = A.lo + (A.hi-A.lo)*i/3, y = Y(val);
...
$('#foodGo').onclick = async ()=>{
  if(!db.n8nUrl) return toast('Zuerst im Daten-Tab die n8n Webhook-URL eintragen');
  const text = $('#foodTxt').value.trim();
  if(!text) return toast('Erst eingeben, was du gegessen hast');
  const btn = $('#foodGo');
  ...
  try{
    const r = await fetch(db.n8nUrl, {
      ...
      body: JSON.stringify({text, date: $('#bdate').value||TODAY})
    });
    if(!r.ok) throw new Error('HTTP '+r.status);
    const d = await r.json();
    const kc = num(d.kcal), pr = num(d.protein);
    if(kc==null && pr==null) throw new Error('Antwort enthält kein kcal/protein');
    if(kc!=null) $('#kcal').value = Math.round((num($('#kcal').value)||0) + kc);
    if(pr!=null) $('#prot').value = Math.round((num($('#prot').value)||0) + pr);
    $('#foodInfo').textContent = `„${text}": ${kc!=null?Math.round(kc)+' kcal':''}${kc!=null&&pr!=null?' · ':''}${pr!=null?Math.round(pr)+' g Protein':''} — oben addiert. Mit „Eintrag speichern" übernehmen.`;
    $('#foodTxt').value = '';
  }catch(err){
    toast('API-Fehler: '+err.message);
  }
  btn.disabled = false; btn.textContent = 'Kalorien berechnen';
};
...
function renderExList(){
  $('#exlist').innerHTML = db.exercises.map(e=>{
    const n = db.workouts.filter(w=>w.exercise===e).length;
    return `<li><div class="li-main"><div class="li-t">${esc(e)}<span class="daytag">${muscleOf(e)}</span></div><div class="li-s">${n} Einheiten</div></div>
...
$('#impJson').onchange = async e=>{
  ...
  try{
    ...
    db = {exercises:d.exercises?.length?d.exercises:[...DEFAULT_EX], workouts:d.workouts, body:d.body, splits:d.splits||[], exGroups:d.exGroups||{}, n8nUrl:d.n8nUrl||db.n8nUrl||''};
