...
let db = {exercises:[...DEFAULT_EX], workouts:[], body:[], splits:[], exGroups:{}, exNotes:{}, customBarcodes:{}, n8nUrl:'', mealTypes:[...DEFAULT_MEALS], goals:{...DEFAULT_GOALS}, foodFav:[], nutrition:{...DEFAULT_NUTRITION}};
...
function weightChart(pts){
  ...
  const W=520, H=232, PL=8, PR=42, PT=30, PB=22;
  ...
  pts.forEach((p,i)=>{
    ...
    if(labelSet.has(i)) g+='<text x="'+cx+'" y="'+(Y(p.y)-9).toFixed(1)+'" text-anchor="middle" font-family="var(--mono)" font-size="10.5" font-weight="600" fill="var(--blue)">'+p.y.toFixed(1)+'</text>';
    g+='<circle class="whit" data-i="'+i+'" cx="'+cx+'" cy="'+cy+'" r="13" fill="transparent"/>';
  });
...
function renderAnalysis(){
  ...
  const wpts = b.map(x=>({x:T(x.date),y:x.weight,date:x.date})).filter(p=>p.y!=null);
  $('#c-weight').innerHTML = weightChart(wpts).svg;
  const _wsel=$('#weightSel');
  const _setWsel=p=>{ if(_wsel) _wsel.innerHTML = p ? '<span class="wsel-w">'+round(p.y,1).toFixed(1)+' kg</span><span class="wsel-d">'+fmtDate(p.date)+'</span>' : '<span class="wsel-hint">Punkt antippen für Tageswert</span>'; };
  _setWsel(wpts.length?wpts[wpts.length-1]:null);
  $$('#c-weight .whit').forEach(c=>c.onclick=()=>{ const p=wpts[+c.dataset.i]; _setWsel(p); $$('#c-weight .wpt').forEach(d=>d.setAttribute('r', d.dataset.i===c.dataset.i?'5.5':'3.6')); });
